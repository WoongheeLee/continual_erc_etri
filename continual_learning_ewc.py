...
def parse_args(args=None):
    ...
    args = parse_args(['--train_root=../data/KEMDy19/', '--valid_root=../data/KEMDy20/'])
    """
...
def ewc_loss_fn(fisher, old_named_params, model, ewc_lambda):
    # Regularization for all previous tasks
    ce_loss_fn = nn.CrossEntropyLoss()

    def _ewc_loss(logits, y_true):
        ce_loss = ce_loss_fn(logits, y_true)
        reg_loss = 0.0
        for (name, param), (_, param_old) in zip(model.named_parameters(), old_named_params.items()):
            reg_loss += torch.sum(fisher[name] * (param_old - param).pow(2)) / 2

        return ce_loss + ewc_lambda * reg_loss
...
def main(args):
    ...
    fisher = build_fisher_matrix(prev_train_ds, model, optimizer, loss_fn, device)
    old_named_params = { name: param.clone().detach()  for name, param in model.named_parameters() }

    loss_fn = ewc_loss_fn(fisher, old_named_params, model, ewc_lambda=args.ewc_lambda)

    best_f1_score = 0.0
    history = {'train': [], 'valid': [], 'prev_valid': []}

    # 학습 시작 전 성능
    valid_outputs = valid_step(valid_dl, model, loss_fn, device)
    prev_valid_outputs = valid_step(prev_valid_dl, model, loss_fn, device)

    history['valid'].append(valid_outputs)
    history['prev_valid'].append(prev_valid_outputs)
    ...
    for epoch in tqdm(range(args.num_epochs), ncols=80, desc='[Epochs]'):
        ...
        print('\n[Epoch {}] train_f1_score: {:.3f}% | valid_f1_score: {:.3f}% | prev_valid_f1_score: {:.3f}%'.format(
            epoch + 1, train_outputs['f1_score'], valid_outputs['f1_score'], prev_valid_outputs['f1_score']))
